...
Slack에서 자연어로 질문하면 SQL로 변환해 evpulse DB를 조회하고 답변한다. 예: "오늘 BSI가 가장 높은 차량 5대 알려줘" → SELECT 생성 → 실행 → 자연어 요약.

chatbot/
├── __init__.py     # Slack 이벤트 라우팅 · 3초 응답 후 비동기 처리
├── gpt_client.py   # 스키마 주입 프롬프트 · 자연어 → SQL → 답변 생성
